...
# -----------------------------
# Utility: Module Impact Analysis
# -----------------------------

def parse_module_impacts(impact_analysis_text, enabled_modules, language):
    """
    Parse the LLM-generated impact analysis to extract summaries for each module.
    
    Parameters:
    - impact_analysis_text: raw text from impact analysis LLM generation
    - enabled_modules: dict of module_key -> bool (only True values relevant)
    - language: current language setting
    
    Returns:
    - dict mapping module_key to impact summary string
    """
    
    impacts = {}
    
    # Get all enabled module keys
    enabled_keys = [k for k, v in enabled_modules.items() if v]
    
    if not enabled_keys:
        return impacts
    
    # Try to extract impact for each module
    for module_key in enabled_keys:
        module_name = recon_assets.get_localized_string(module_key, language)
        
        # Look for the module name or key in the analysis text
        # and try to extract the following 1-2 sentences
        text_lower = impact_analysis_text.lower()
        module_lower = module_name.lower()
        
        # Find position of module name
        module_pos = text_lower.find(module_lower)
        
        if module_pos != -1:
            # Extract from after the module name to the next module or end
            # Look for next module name or natural break
            start_pos = module_pos + len(module_lower)
            
            # Find the next sentence(s) - extract up to 2 sentences or 200 characters
            remaining_text = impact_analysis_text[start_pos:]
            
            # Split by period or newline to get sentences
            sentences = []
            current_sentence = ""
            ...
            # If we have sentences, use them
            if sentences:
                impact_summary = " ".join(sentences[:2]).strip()
                # Clean up the summary - remove leading/trailing punctuation
                impact_summary = impact_summary.strip('.,;:!?\n ')
                impacts[module_key] = impact_summary
            else:
                # Fallback: just take first 150 characters
                fallback = remaining_text[:150].strip('.,;:!?\n ')
                if fallback:
                    impacts[module_key] = fallback
